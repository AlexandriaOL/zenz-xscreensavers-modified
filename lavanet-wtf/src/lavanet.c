...
Color startColor = {.red = 0, .green = 255, .blue = 255};
...
Color cycle_hue(Color in) {
    ...
    if (r == max && g < max && b == t) {
        in.green += delta;
    } else if (r > t && g == max && b == t) {
        in.red -= delta;
    } else if (r == t && g == max && b < max) {
        in.blue += delta;
    } else if (r == t && g > t && b == max) {
        in.green -= delta;
    } else if (r < max && g == t && b == max) {
        in.red += delta;
    } else if (r == max && g == t && b > t) {
        in.blue -= delta;
    }
...
void draw_lines(struct line *lines, int lineCount, Display *dpy, GC g,
    ...
    for (idx = 0; idx < lineCount; idx++) {
        ...
        int delta = line->value/2;
        if (targetRed == 255 && targetGreen < 255 && targetBlue == THRESHOLD) {
            targetGreen -= delta/2;
            targetBlue -= delta;
        } else if (targetRed > THRESHOLD && targetGreen == 255 && targetBlue == THRESHOLD) {
            targetRed -= delta/2;
            targetBlue -= delta;
        } else if (targetRed == THRESHOLD && targetGreen == 255 && targetBlue < 255) {
            targetRed -= delta;
            targetBlue -= delta/2;
        } else if (targetRed == THRESHOLD && targetGreen > THRESHOLD && targetBlue == 255) {
            targetRed -= delta;
            targetGreen -= delta/2;
        } else if (targetRed < 255 && targetGreen == THRESHOLD && targetBlue == 255) {
            targetRed -= delta/2;
            targetGreen -= delta;
        } else if (targetRed == 255 && targetGreen == THRESHOLD && targetBlue > THRESHOLD) {
            targetGreen -= delta;
            targetBlue -= delta/2;
        }
        ...
        /*                make_color(targetRed * line->value, */
        /*                           targetGreen * line->value, */
        /*                           targetBlue * line->value)); */
        /* This 'works' but is blocky: */
        ...
        for (idx2 = idx + 1; idx2 < pointCount; idx2++) {
            ...
            if (distance < minimumDistance) {
                ...
                *lines = realloc(*lines, counter * sizeof(struct line));
                ...
                newLine->value = (int)floor(distance / minimumDistance * 256);
            }
        }
    }
