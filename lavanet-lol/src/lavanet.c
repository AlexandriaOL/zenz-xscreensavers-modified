 ...
 *
 *      Edited 2020-09-12 by Alexandria O'Leary
 *
 *      Modified to cycle through neon colours.
 */
...
ColorTarget make_random_color(void)
{
	...
	ColorTarget ret = {
		.red = (mixTarget.red + random_red)/2,
		.green = (mixTarget.green + random_green)/2,
		.blue = (mixTarget.blue + random_blue)/2
	};
...
void draw_lines(struct line *lines, int lineCount, Display *dpy, GC g,
	...
	for (idx = 0; idx < lineCount; idx++) {
		...
		ColorTarget targetColor = make_random_color();
